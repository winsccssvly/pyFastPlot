...
class PlotOptionsWidget(QWidget):
    ...
    def __init__(self):
        ...
        self.applied_font_label.setStyleSheet(
        ...
        self.title_size.setValue(DEFAULT_TITLE_SIZE)
        ...
        self.label_size.setValue(DEFAULT_LABEL_SIZE)
        ...
        self.tick_size.setValue(DEFAULT_TICK_SIZE)
        ...
        self.legend_size.setValue(DEFAULT_LEGEND_SIZE)
        ...
        global_layout.addRow(
            "X Min/Max:", self._create_minmax_layout(self.xmin_input, self.xmax_input)
        )
        global_layout.addRow(
            "Y Min/Max:", self._create_minmax_layout(self.ymin_input, self.ymax_input)
        )
    ...
    def get_line_options(self, row):
        try:
            ...
            if (
                isinstance(color_w, QComboBox)
                and isinstance(style_w, QComboBox)
                and isinstance(width_w, QDoubleSpinBox)
                and isinstance(marker_w, QComboBox)
                and isinstance(msize_w, QDoubleSpinBox)
                and isinstance(fill_w, QComboBox)
            ):
                return {
                    "color": color_w.currentText()
                    if color_w.currentText() != "auto"
                    else "",
