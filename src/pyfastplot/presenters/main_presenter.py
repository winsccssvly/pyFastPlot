...
class MainPresenter:
    ...
    It listens to UI signals, updates the Model, and refreshes the View.
    """
    ...
    def _connect_signals(self):
        ...
        self.view.plot_options.line_table.cellChanged.connect(
            self.on_cart_label_changed
        )
    ...
    def on_save_plot(self):
        file_path, _ = QFileDialog.getSaveFileName(
            self.view, "Save Plot", "", "PNG Image (*.png);;SVG Image (*.svg)"
        )
        ...
        if file_path:
            ...
            try:
                ...
                QMessageBox.critical(
                    self.view, "Error", f"An error occurred while saving the file:\n{e}"
                )
    ...
    def on_add_input_table(self):
        ...
        items = self.view.table_list_widget.findItems(
    ...
    def on_delete_table(self, table_name):
        ...
        items = self.view.table_list_widget.findItems(
            table_name, Qt.MatchFlag.MatchExactly
        )
        if items:
            self.view.table_list_widget.takeItem(
                self.view.table_list_widget.row(items[0])
            )
    ...
    def on_reload_table(self, table_name):
        ...
        if success:
            ...
            QMessageBox.warning(
                self.view,
                "Warning",
                "The original file path could not be found or the file does not exist.",
            )
    ...
    def on_data_pasted(self, start_row, start_col, parsed_data):
        ...
        if not success:
            QMessageBox.warning(
                self.view,
                "Warning",
                "No table is selected. Please add or select a table "
                "from the left panel.",
            )
            return
        self._update_view_for_current_table()
    ...
    def _add_to_cart(self, clear_first):
        ...
        if not y_col_name:
            QMessageBox.warning(
                self.view, "Warning", "Please select the Y data to add to the cart."
            )
            return False
        ...
        for i in range(min_len):
            ...
            if (isinstance(xv, float) and np.isnan(xv)) or (
                isinstance(yv, float) and np.isnan(yv)
            ):
                continue
        ...
        self.model.add_to_cart(
            table_name, x_label, y_label, np.array(clean_x), np.array(clean_y)
        )
        self.view.plot_options.add_cart_item(y_label, table_name)
        return True

    def on_plot_data(self):
        # Suppress fallback font warnings that clutter the console.
        ...
        has_korean = any(
            bool(re.search(r"[가-힣ㄱ-ㅎㅏ-ㅣ]", str(t))) for t in displayed_texts if t
        )
        ...
        self.view.aspect_wrapper.set_aspect_ratio(
            settings["fig_width"], settings["fig_height"]
        )
        ...
        self.view.plot_options.applied_font_label.setText(
            f"Applied Font: {actual_font}"
        )
